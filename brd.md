...
* Должна быть возможность создавать тип секций "Перечисление", что идентифицирует секцию как перечисление полей, где
  ...
  а) секцией или б) просто строкой и может (а может и быть пустым) иметь значение типа [списка полей разного типа](#list_dif_fields)
...
* По умолчанию должны быть эти типы полей с уникальным интерфейсом, где + означает может ли быть списком:
    ...
    * <a name="list_dif_fields"></a>-**Список полей разного типа**: Содержит несколько полей разных типов, разделенных запятыми. Важно чтобы у каждого
