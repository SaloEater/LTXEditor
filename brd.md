# Документ с Бизнес-Требованиями (BRD) для Редактора LTX конфигов S.T.A.L.K.E.R.: Зов Припяти

## 1. Цели и Задачи Проекта

**Цель:** Создать интерактивный редактор конфигов для игры S.T.A.L.K.E.R.: Зов Припяти.
...
2) Не забыть про то, что инфопоршни еще лежат в xml диалогах. Там такой же подход, но еще проще, т.к. там заранее
   определены все теги
3) В теории еще круто делать поля-агреггаты где несколько полей настраиваются в одном интерфейсе, например, координаты
   брони в инвентаре:
4) Можно ли подружить с каким-нибудь редактором типо Idea/VSCode чтобы можно было туда ссылаться на исходники скриптов
...
1. Создать гибкую систему для управления типами секций и типов полей в них. Такая система будет настраиваться из
   внешнего файла для того, чтобы не нужно было рекомпилировать редактор при добавлении новых типов секций. А новые 
    типы будут создавать изнутри редактора
...
* Должен быть тип "Неизвестно", который будет использоваться для полей, которые не поддерживаются редактором, но могут
  быть в секции
* <Поддержка DLTX>
* По умолчанию должны быть эти типы полей с уникальным интерфейсом:
    * **Неизвестно**: Все значение поля будет принимать как одна строка - `a = literally anything`
    * **Список Неизвестно**: Нескольких строк, разделенных запятыми - `a = literally anything, literally anything else`
    ...
    * **Список файлов из одной из папок из gamedata**: Поле для выбора относительного пути до нескольких файлов из одной
      из папок в gamedata
    * **Прогрессия**: Обозначает прогрессию - `rating = novice, 300, experienced, 600, veteran, 900, master`
    * **Нумерованный список**: Список со значениям где каждому придается какой-то номер -
    ...
    * **Список секций**: `npc_random = sim_default_duty_0, sim_default_duty_1`
