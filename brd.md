...
1) Можно ли в DLTX конфигах заранее знать какой у этого файла root file который нужно перезаписать для изменения 
выбранной секции?
2) Не забыть про то, что инфопоршни еще лежат в xml диалогах. Там заранее определены теги по которым можно парсить
   поршни
3) Можно ли подружить с каким-нибудь редактором типо Idea/VSCode чтобы можно было туда ссылаться на исходники скриптов
4) В теории может понадобиться делать поля-агреггаты где несколько полей настраиваются в одном интерфейсе, например, координаты
   брони в инвентаре:
...
1. Создать гибкую систему для управления типами секций и типов полей в них. Во внешнем файле будут описываться секции с
   набором их полей и типов этих полей. А внутри самого редактора можно будет добавлять функционал для новых типов
   полей.
2. Создать сам редактор.
3. Протестировать редактор на ЗП и уже готовых модах.
4. Протестировать редактор на сборках типо гаммы

## 2. Описание Проблемы

* Сейчас не существует удобного инструмента для редактирования конфигов всего мода
* Сейчас не существует удобного инструмента для редактирования конфигов всей сборки модов
...
* Ну и просто смертельно надоело прыгать по конфигам через поиск, а не через контрл+ЛКМ.
...
### 5.0. Главная задача редактора

1) Показать какие секции доступны в игре/моде/сборке
2) Показать какие поля могут быть в секции
3) Показать какие значения принимают поля секции
4) Наглядный редактор DLTX

### 5.1. Работа с файлами

#### 5.1.1 Инициализация файловой системы

Для поддержки модов и сборок нужна возможность подгружать сразу несколько gamedata с возможностью создавать, сохранять и
удалять в них файлы

#### 5.1.2 Возможности работы с файлами

* Возможность подгружать несколько папок gamedata
* Возможность идентифицировать из какой gamedata был открыт файл
...
* Возможность обозначать файл как "логика". Для таких конфигов будет использоваться
  специальный [интерфейс](#541-визуализация-и-редактирование-logic-конфигов)
...
* Должна быть возможность создавать тип секций "Перечисление", что идентифицирует секцию как перечисление полей, где
  каждое поле будет представлять собой отдельный элемент этого перечисления с именем, которое может быть
  а) секцией или б) просто строкой и может (а может и быть пустым) иметь значение
  типа [списка полей разного типа](#list_dif_fields)
...
* По умолчанию должны быть эти типы полей с уникальным интерфейсом, где + означает может ли быть списком:
    ...
    * +**Файл из одной из папок из gamedata**: Поле для выбора относительного пути файла из одной из папок в gamedata с
      флагом "Без формата". **Пример**: `blood = wm\wm_blood_1` - где `wm\wm_blood_1.thm` - это относительный путь до
      файла
    ...
    * <a name="list_dif_fields"></a>-**Список полей разного типа**: Содержит несколько полей разных типов, разделенных
      запятыми. Важно чтобы у каждого
...
* Визуально связывает между собой секции на основе полей с типом **Секция**
...
Благодаря парсингу кондлистов и xml диалогов, мы можем сделать примерное дерево инфопоршней, по которому можно будет
иерархически связать между собой скрипты-конфиги, поведение нпс, диалоги и все остальный файлы этой мега-системы.
...
1) Подсказка в поле для секции
   1) Секции из этого же файла
   2) Секции название которых похоже на поисковой запрос
   3) Секции из остальных файлов
2) Навигация по секциям
   1) Где эта секция используется/объявляется в этом файле
   2) Где эта секция используется/объявляется в других файлах
